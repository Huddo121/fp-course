...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok lc rc = between (charTok lc) (charTok rc)
...
hex ::
  Parser Char
hex = chr . getHex <$> replicateA 4 (satisfy isHexDigit)
  where getHex s = readHex s ?? 0
-- hex = replicateA 4 (satisfy isHexDigit)
  -- replicateA 4 (satisfy isHexDigit) :: Parser (List Char)
  -- replicateA :: Int -> f a -> f (List a)
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby pa ps = sepby1 pa ps ||| list pa
...
eof ::
  Parser ()
eof = P (\i -> bool (UnexpectedString i) (Result i ()) (i == ""))
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll preds = P (\i -> (sequence $ satisfy <$> preds) i)
